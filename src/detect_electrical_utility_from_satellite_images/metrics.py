...
def calculate_iou(
    ...
    cm = torch.zeros((num_classes, num_classes), dtype=torch.int64, device=predictions.device)
    ...
    if average == "none":
        ...
        return (total_intersection.float() / total_union.float()).item() if total_union > 0 else 0.0
...
def calculate_dice(
    ...
    if average == "none":
        ...
        total_intersection = torch.sum(torch.diag(cm))
        total = torch.sum(cm)
        return (2 * total_intersection.float() / total.float()).item() if total > 0 else 0.0
    elif average == "weighted":
        class_counts = torch.sum(cm, dim=1)
        weights = class_counts.float() / torch.sum(class_counts).float()
...
def calculate_precision_recall_f1(
    ...
    for c in range(num_classes):
        ...
        precision = tp.float() / (tp + fp) if (tp + fp) > 0 else torch.tensor(0.0, device=predictions.device)
        recall = tp.float() / (tp + fn) if (tp + fn) > 0 else torch.tensor(0.0, device=predictions.device)
        f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else torch.tensor(0.0, device=predictions.device)
    ...
    if average == "none":
        ...
        micro_precision = total_tp.float() / (total_tp + total_fp) if (total_tp + total_fp) > 0 else torch.tensor(0.0, device=predictions.device)
        micro_recall = total_tp.float() / (total_tp + total_fn) if (total_tp + total_fn) > 0 else torch.tensor(0.0, device=predictions.device)
        micro_f1 = 2 * micro_precision * micro_recall / (micro_precision + micro_recall) if (micro_precision + micro_recall) > 0 else torch.tensor(0.0, device=predictions.device)
